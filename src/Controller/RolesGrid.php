...
class RolesGrid extends DialogGridController
{
    ...
    /**
     * Идентификатора панели виджетов.
     * 
     * @var int
     */
    ...
    public function translateAction(mixed $params, string $default = null): ?string
    {
        switch ($this->actionName) {
            // вывод интерфейса
            case 'view':
                /** @var \Gm\Session\Container $storage Хранилище модуля */
                $storage = $this->module->getStorage();
                /** @var null|array $panel */
                $panel = $storage->panel;
                return $this->module->t(
                    'opening a window for viewing user roles available to the partition {0}', [$panel['name'] ?? SYMBOL_NONAME]
                );
            ...
            case 'data':
                ...
                $panel = $storage->panel;
                return $this->module->t(
                    'viewing user roles available to the partition {0}', [$panel['name'] ?? SYMBOL_NONAME]
                );
    ...
    /**
     * Возвращает идентификатор выбранной панели виджетов.
     * 
     * @return int
     */
    public function getPanelIdentifier(): int
    {
    ...
    {
        /** @var \Gm\Backend\Dashboard\Model\Dashboard|null $dashboardAR */
        $dashboardAR = $this->module->getModel('Dashboard');

        /** @var \Gm\Backend\Dashboard\Model\Dashboard|null $panelAR Панель виджетов */
        $panelAR = $dashboardAR->selectByPk($this->getPanelIdentifier());
        if ($panelAR === null)
            return $this->getResponse()->error($this->t('Incorrect partition panel'));

        /** @var \Gm\Session\Container $storage Хранилище модуля */
        $storage = $this->module->getStorage();
        $storage->panel = $panelAR->getAttributes();
        ...
        $window->title = $this->module->t('{roles.title}', [$panelAR->name ?? '']);
        $window->requires[] = 'Gm.view.grid.column.Switch';
